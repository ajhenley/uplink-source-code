...
/**
 * Terminal-style password entry screen.
 *
 * Displayed when a remote computer requires authorization.
 * The player types a password (shown as asterisks) and presses Enter
 * or clicks SUBMIT. A BYPASS button is provided as a placeholder for
 * the future password-breaker hacking tool.
 *
 * All game objects are added to `container` so the parent scene can
 * manage their lifecycle.
 */
export class PasswordScreen {
  private scene: Phaser.Scene;
  private container: Phaser.GameObjects.Container;
  private typedPassword: string = '';
  ...
  private buildUI(screenData: any) {
    ...
    divider.lineBetween(boxX + 20, boxY + 55, boxX + boxW - 20, boxY + 55);
    this.container.add(divider);

    // ── Prompt label ──
    const promptLabel = screenData.prompt || 'Enter Password:';
    ...
    // ── Password input field (background + text) ──
    const fieldX = boxX + 30;
    const fieldY = boxY + 105;
    const fieldW = boxW - 60;
    const fieldH = 28;

    const fieldGfx = this.scene.add.graphics();
    fieldGfx.fillStyle(CONFIG.COLORS.BLACK, 1);
    ...
    this.passwordDisplay = this.scene.add.text(
      fieldX + 8,
      fieldY + 7,
      '_',
      {
    ...
    this.container.add(this.passwordDisplay);
    ...
    this.createButton('BYPASS', contentWidth / 2 + 60, btnY, 120, 30, () => {
      // Placeholder for password-breaker tool integration
      console.log('[PasswordScreen] BYPASS button pressed -- not yet implemented');
    }, true);
  ...
  private createButton(
    ...
    const text = this.scene.add.text(x, y, label, {
      fontFamily: 'Courier New',
      fontSize: '12px',
      color: dimmed ? CONFIG.COLOR_STR.GREEN : CONFIG.COLOR_STR.GREEN,
    }).setOrigin(0.5);
    if (dimmed) {
      text.setAlpha(0.4);
    }
  ...
  private handleKey(event: KeyboardEvent) {
    ...
    if (event.key === 'Backspace') {
      this.typedPassword = this.typedPassword.slice(0, -1);
      this.updatePasswordDisplay();
      return;
    }
    ...
    if (event.key.length === 1 && !event.ctrlKey && !event.metaKey) {
      const maxLen = 32;
      if (this.typedPassword.length < maxLen) {
        this.typedPassword += event.key;
        this.updatePasswordDisplay();
      }
    }
  }
  ...
  private updatePasswordDisplay() {
    const masked = '*'.repeat(this.typedPassword.length);
    const cursor = this.cursorVisible ? '_' : '';
    this.passwordDisplay.setText(masked + cursor);
  }

  // ── Actions ──────────────────────────────────────────────────────

  private submit() {
    if (this.typedPassword.length === 0) {
      this.errorText.setText('Error: Please enter a password');
      return;
    }
